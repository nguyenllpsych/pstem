---
title: "P-STEM Wave 1 Target Codebook"
author: "Linh Nguyen"
date: "`r Sys.Date()`"
output: 
  html_document:
...
This [codebook](https://rpubs.com/nguyenllpsych/pstem_target1) includes only quantitative scales. Qualitative responses collected for the following scales are not included here:
...
- Year in school (open ended)
- Previous institution if transferred (open ended)
- Major (open ended)
...
```{r load-data}
# load scored data
data <- readRDS(file = "../PSTEM_Wave_1_SCORED_2023-02-21.RData")
...
ggplot(data = data, aes(age)) +
  ...
  geom_vline(xintercept = mean(data$age, na.rm = T),
             color = "#f1c232", size = 1) +
  annotate("text", 
           label = paste0("Mean = ", round(mean(data$age, na.rm = T), 3),
                          ", SD = ",round(sd(data$age, na.rm = T), 3)),
           x = 25, y = 50) + 
  labs(
...
# social class
ggplot(data[which(!is.na(data$socialclass)),], 
       aes(factor(socialclass,
                  levels = c(1,2,3,4,5),
                  labels = c("Low-income/poor", "Working class/Lower middle class",
                             "Middle class", "Upper middle class",
                             "Upper class")))) + 
  geom_bar(fill = "#710c0c") + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of social class background",
    x = NULL
  ) +
  ylim(0, 100) +
  theme_classic() +
...
# transfer
ggplot(data[which(!is.na(data$transfer)),], 
       aes(factor(transfer,
                  levels = c(1,2),
                  labels = c("No","Yes")))) + 
  geom_bar(fill = c("#f1c232", "#710c0c")) + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of transfer students",
    x = NULL
  ) +
  ylim(0, 175) +
  theme_classic()
...
# intl
ggplot(data[which(!is.na(data$intl)),], 
       aes(factor(intl,
                  levels = c(1,2),
                  labels = c("No","Yes")))) + 
  geom_bar(fill = c("#f1c232", "#710c0c")) + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of international students",
    x = NULL
  ) +
  ylim(0, 200) +
  theme_classic()
...
# usborn
ggplot(data[which(!is.na(data$usborn)),], 
       aes(factor(usborn,
                  levels = c(1,2),
                  labels = c("No","Yes")))) + 
  geom_bar(fill = c("#f1c232", "#710c0c")) + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of US-born students",
    x = NULL
  ) +
  ylim(0, 175) +
  theme_classic()
...
# usborn_p
ggplot(data[which(!is.na(data$usborn_p)),], 
       aes(factor(usborn_p,
                  levels = c(1,2,3),
                  labels = c("Neither parent","One parent","Both parents")))) + 
  geom_bar(fill = c("#f1c232","#710c0c","#710c0c")) + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of US-born parents",
    x = NULL
  ) +
  ylim(0, 175) +
  theme_classic()
...
# liveus
ggplot(data[which(!is.na(data$liveus)),], 
       aes(factor(liveus,
                  levels = c(1,2),
                  labels = c("No","Yes")))) + 
  geom_bar(fill = c("#f1c232","#710c0c")) + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of US-based students",
    x = NULL
  ) +
  ylim(0, 225) +
  theme_classic()
...
# politics
ggplot(data[which(!is.na(data$politics)),], 
       aes(factor(politics,
                  levels = c(1:7),
                  labels = names(val_labels(data$politics))))) + 
  geom_bar(fill = c("#710c0c")) + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of political views",
    x = NULL
  ) +
  ylim(0, 90) +
  theme_classic() +
...
# religion
ggplot(data[which(!is.na(data$religion)),], 
       aes(factor(religion,
                  levels = c(1:4),
                  labels = names(val_labels(data$religion))))) + 
  geom_bar(fill = c("#710c0c")) + 
  geom_text(stat='count', aes(label=..count..), vjust = -1) + 
  labs(
    title = "Count of religious views",
    x = NULL
  ) +
  ylim(0, 80) +
  theme_classic()
...
# covid_1
ggplot(data = data, aes(covid_1)) +
  geom_histogram(binwidth = 1, fill = "#710c0c") + 
  geom_vline(xintercept = mean(data$covid_1, na.rm = T),
             color = "#f1c232", size = 1) +
  annotate("text", 
           label = paste0("Mean = ", round(mean(data$covid_1, na.rm = T), 3),
                          ", SD = ",round(sd(data$covid_1, na.rm = T), 3)),
           x = 1, y = 25) + 
  labs(
    title = "Distribution of COVID stress",
    x = "COVID stress (0-10 scale)",
    y = NULL
  ) +
  theme_classic()
```

```{r english, fig.width=11, fig.height = 7}
# english comfort
english <- dict %>%
  filter(grepl(pattern = "english", x = variable)) %>%
  pull(variable)
english <- data %>%
  select(all_of(english)) 
names(english) <- var_label(english)
english <- english %>%
  mutate_all(.funs = function(x){
    factor(x, levels = c(1:4),
           labels = c("Beginning","Intermediate","Proficient","Advanced"))
  })
plot(likert(english)) + 
...
## Big Five Aspect Scale - Domain Level (DeYoung et al., 2007) 

```{r bfas-domain, fig.width=11, fig.height = 10}
# items
bfas_variables <- c("agreeableness", "conscientiousness", "extraversion",
                    "neuroticism", "openness")
for(var in bfas_variables){
  items <- dict %>% 
    filter(scale == var) %>% 
    pull(variable)
  items <- data %>%
  ...
  print(plot(likert(items),
       main = var) + 
    theme(axis.text = element_text(size=12)) +
    labs(title = paste0("BFAS Domain: ", var)))
}
```

## Big Five Aspect Scale - Aspect Level (DeYoung et al., 2007) 

```{r bfas-aspect, fig.width=11}
# items
bfas_variables <- c("compassion", "politeness",
...
for(var in bfas_variables){
  ...
  print(plot(likert(items),
       main = var) + 
    theme(axis.text = element_text(size=12)) +
    labs(title = paste0("BFAS Aspect: ", var)))
}
```
...
```{r iq, fig.width=11, fig.height = 20}
iq_items <- c("iq_1_scored", "iq_2_scored", "iq_3_scored", "iq_4_scored", 
              "iq_5_scored", "iq_6_scored", "iq_7_scored", "iq_8_scored", 
              "iq_9_scored", "iq_10_scored", "iq_11_scored", "iq_12_scored", 
              "iq_13_scored", "iq_14_scored", "iq_15_scored", "iq_16_scored")
iq_items <- data %>%
  select(all_of(iq_items))
iq_ori <- c("iq_1", "iq_2", "iq_3", "iq_4", 
            "iq_5", "iq_6", "iq_7", "iq_8", 
            "iq_9", "iq_10", "iq_11", "iq_12", 
            "iq_13", "iq_14", "iq_15", "iq_16")
iq_ori <- data %>%
  select(all_of(iq_ori))
names(iq_items) <- var_label(iq_ori)
iq_items <- iq_items %>%
  mutate_all(.funs = function(x){
    factor(x, levels = c(0,1),
           labels = c("Incorrect", "Correct"))
  })
plot(likert(iq_items)) + 
  theme(axis.text = element_text(size=12))
```
...
## Interest Inventory (Armstrong et al., 2008)

```{r ii, fig.width=11}
ii_variables <- c("realistic", "investigative", "artistic", 
                  "social", "enterprising", "conventional")
for(var in ii_variables){
  items <- dict %>% 
    filter(scale == var) %>% 
    pull(variable)
  items <- data %>%
  ...
  mutate_all(.funs = function(x){
    factor(x, levels = c(1:5),
           labels = c("Strongly Dislike", "Dislike", 
                      "Neutral", "Like", "Strongly Like"))
  })
  print(plot(likert(items),
       main = var) + 
    theme(axis.text = element_text(size=12)) +
    labs(title = paste0("Interest Dimension: ", var)))
}
```
